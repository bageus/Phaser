...
function getSegmentColor(angle, index, colorBoost) {
  ...
  const r = clamp(
    Math.round(110 + Math.sin((hue * Math.PI) / 180) * 16 + colorBoost * 26),
    0,
    255,
  );
  const g = clamp(
    Math.round(36 + Math.cos((hue * Math.PI) / 180) * 10 + colorBoost * 12),
    0,
    255,
  );
  const b = clamp(
    Math.round(84 + Math.sin((hue * Math.PI) / 360) * 34 + colorBoost * 48),
    0,
    255,
  );
  return rgbToInt(r, g, b);
...
function boostPulseFromSpeed(speed) {
  return clamp(
    (speed - BOOST_THRESHOLD) /
      Math.max(0.0001, CONFIG.SPEED_MAX - BOOST_THRESHOLD),
    0,
    1,
  );
}
...
class TunnelRenderer {
  ...
  updateFxState() {
    ...
    const quality =
      QUALITY_PRESETS[tube?.quality || "high"] || QUALITY_PRESETS.high;
    if (!tube) {
    ...
    const offsetMagnitude = Math.hypot(
      tube.centerOffsetX || 0,
      tube.centerOffsetY || 0,
    );
    const speedDelta = Math.max(0, tube.speed - this.prevSpeed);
    const curveDelta = Math.abs(
      (tube.curveStrength || 0) - this.prevCurveStrength,
    );
    ...
    this.flashLevel = clamp(
      this.flashLevel * quality.flashDecay + curveDelta * 3.8 + alertImpulse,
      0,
      1.1,
    );
    this.rippleLevel = clamp(
      this.rippleLevel * quality.rippleDecay +
        offsetDelta / 18 +
        curveDelta * 1.2,
      0,
  ...
  drawStructuralRing(
    ...
    const mouthRadius = CONFIG.TUBE_RADIUS;
    ...
    const ringGlow = clamp(
      0.16 + boostRatio * 0.14 + this.boostPulse * 0.18,
      0.14,
      0.48,
    );
    const frameColor = blendColor(
      0x211629,
      tintColor,
      player?.shield ? 0.4 : 0.24,
    );
    const textureStroke = blendColor(
      0x8fd5ff,
      tintColor,
      0.3 + ((fx?.x2Timer || 0) > 0 ? 0.08 : 0),
    );
    const textureRows = qualityName === "low" ? 2 : 3;
    ...
    this.baseGraphics.strokeEllipse(
      ...
      centerY,
      outerRadius * 2.08,
      outerRadius * 2.08 * CONFIG.PLAYER_OFFSET,
    );
    ...
    this.baseGraphics.strokeEllipse(
      ...
      centerY,
      midRadius * 2,
      midRadius * 2 * CONFIG.PLAYER_OFFSET,
    );
    this.baseGraphics.lineStyle(
      qualityName === "low" ? 1 : 2,
    ...
    for (let i = 0; i < panelCount; i += 1) {
      const angle = (Math.PI * 2 * i) / panelCount;
      ...
      const panelColor =
        i % 2 === 0
          ? blendColor(0x5cf2ff, tintColor, 0.5)
      ...
      this.lightGraphics.moveTo(
        ...
        centerY + Math.sin(angle) * innerRadius * CONFIG.PLAYER_OFFSET,
      );
      this.lightGraphics.lineTo(
        centerX + Math.cos(angle) * midRadius,
        centerY + Math.sin(angle) * midRadius * CONFIG.PLAYER_OFFSET,
      );
      this.lightGraphics.strokePath();

      this.lightGraphics.lineStyle(1, textureStroke, 0.12 + panelAlpha * 0.26);
      for (let row = 0; row < textureRows; row += 1) {
        const rowStart = 0.16 + row * 0.22;
        const rowEnd = Math.min(0.94, rowStart + 0.24);
        this.lightGraphics.beginPath();
        this.lightGraphics.moveTo(
          lerp(p4x, p1x, rowStart),
          lerp(p4y, p1y, rowStart),
        );
        this.lightGraphics.lineTo(
          lerp(p3x, p2x, rowEnd),
          lerp(p3y, p2y, rowEnd),
    ...
    const rimColor = blendColor(
      0x8fd5ff,
      tintColor,
      0.4 + ((fx?.x2Timer || 0) > 0 ? 0.08 : 0),
    );
    this.lightGraphics.lineStyle(
      qualityName === "low" ? 2 : 6,
      rimColor,
    ...
    this.lightGraphics.strokeEllipse(
      ...
      centerY,
      innerRadius * 2.01,
      innerRadius * 2.01 * CONFIG.PLAYER_OFFSET,
    );
    this.lightGraphics.lineStyle(
      qualityName === "low" ? 1 : 3,
    ...
    this.lightGraphics.strokeEllipse(
      ...
      centerY,
      innerRadius * 1.98,
      innerRadius * 1.98 * CONFIG.PLAYER_OFFSET,
    );
  }

  drawInteriorSpokes(centerX, centerY, tintColor, qualityName, boostRatio) {
    const spokeCount =
      qualityName === "low" ? 14 : qualityName === "medium" ? 22 : 30;
    const maxRadius = CONFIG.TUBE_RADIUS * 0.92;
    const spokeAlpha = clamp(
      0.12 + boostRatio * 0.16 + this.flashLevel * 0.12,
      0.1,
      0.34,
    );
    const spokeColor = blendColor(0x7d1e44, tintColor, 0.12);

    this.fogGraphics.lineStyle(
    ...
    for (let i = 0; i < spokeCount; i += 1) {
      const angle =
        (Math.PI * 2 * i) / spokeCount - this.snapshot.tube.rotation * 0.24;
  ...
  drawLampGlow(centerX, centerY, tube, tintColor, qualityName) {
    ...
    for (let i = 0; i < lampCount; i += 1) {
      ...
      this.lightGraphics.fillRoundedRect(
        x - width / 2,
        y - height / 2,
        width,
        height,
  ...
  drawTunnel() {
    ...
    const boostRatio = clamp(
      (tube.speed - CONFIG.SPEED_START) /
        Math.max(0.0001, CONFIG.SPEED_MAX - CONFIG.SPEED_START),
      0,
      1.6,
    );
    ...
    const shadowMagnitude = Math.hypot(
      tube.centerOffsetX || 0,
      tube.centerOffsetY || 0,
    );
    const mouthCenterX = centerX;
    const mouthCenterY = centerY;
    const depthFogStrength = clamp(
      0.16 + boostRatio * 0.18 + this.boostPulse * 0.08,
      0.1,
      0.42,
    );
    ...
    for (let depth = maxDepth - 1; depth >= 0; depth -= quality.depthStep) {
      ...
      for (let i = 0; i < segmentCount; i += quality.segmentStep) {
        ...
        const boundaryB =
          ...
          tube.curveAngle;
        const segmentMid =
          ((i + quality.segmentStep * 0.5) / segmentCount) * Math.PI * 2 +
          tube.rotation;
        ...
        const shadowDiff = Math.abs(
          normalizeAngleDiff(segmentMid - shadowAngle),
        );
        const shadowFactor =
          shadowMagnitude > 0
            ? clamp(1 - shadowDiff / (Math.PI * 1.16), 0, 1)
            : 0;
        const shadowAlpha = clamp(
          (shadowMagnitude / 110) * shadowFactor * (0.7 + depthRatio),
          0,
          0.42,
        );
        ...
        if (player?.magnetActive)
          fillColor = blendColor(fillColor, 0x7dffb3, 0.16);
        if ((fx?.x2Timer || 0) > 0)
        ...
        if (lampFactor > 0.015 && edgeHighlight > 0.15) {
          ...
          this.lightGraphics.lineStyle(
            qualityName === "high" ? 3 : 2,
            blendColor(0xa9ecff, tintColor, 0.48),
            edgeGlowAlpha,
          );
          this.lightGraphics.beginPath();
    ...
    for (let i = 0; i < quality.fogLayers; i += 1) {
      ...
      this.fogGraphics.lineStyle(
        qualityName === "low" ? 1 : 2,
        blendColor(0x150c22, tintColor, 0.14 + t * 0.12),
        0.04 + t * 0.05,
      );
      this.fogGraphics.strokeEllipse(
        mouthCenterX,
        mouthCenterY,
        radius * 2,
        radius * 2 * CONFIG.PLAYER_OFFSET,
      );
    }

    this.drawInteriorSpokes(
      mouthCenterX,
      mouthCenterY,
      tintColor,
      qualityName,
    ...
    if (this.flashLevel > 0.03) {
      this.flashGraphics.fillStyle(
        blendColor(0xff5aa9, tintColor, 0.2),
        clamp(this.flashLevel * 0.18, 0.03, 0.2),
      );
      this.flashGraphics.fillEllipse(
        mouthCenterX,
        mouthCenterY,
        CONFIG.TUBE_RADIUS * 2.2,
        CONFIG.TUBE_RADIUS * 2.2 * CONFIG.PLAYER_OFFSET,
      );
    }

    if (this.rippleLevel > 0.04) {
      const rippleRadius =
        CONFIG.TUBE_RADIUS * (0.36 + this.rippleLevel * 0.75);
      this.flashGraphics.lineStyle(
        qualityName === "low" ? 1 : 2,
        blendColor(0xffffff, tintColor, 0.45),
        clamp(this.rippleLevel * 0.26, 0.04, 0.3),
      );
      this.flashGraphics.strokeEllipse(
        mouthCenterX,
        mouthCenterY,
        rippleRadius * 2,
        rippleRadius * 2 * CONFIG.PLAYER_OFFSET,
      );
    }
  }
